...
def lower_wedge():
    ...
    motor_tracao.run_until_stalled(speed=500, then=Stop.HOLD, duty_limit=50)
    # Lift the wedge just a bit to avoid dragging it on the floor and losing traction
    motor_tracao.run_angle(speed=500, rotation_angle=-10, then=Stop.HOLD, wait=False)
...
def check_boundaries():
    ...
    while True:
        ...
        if value_L >= (cor_chao + EDGE_THRESHOLD):
            ...
            motor_tracao.run_time(speed=-500, time=500)
...
motor_tracao = Motor(Port.C, Direction.CLOCKWISE)

sensor_cor = ColorSensor(Port.S1) # ESQUERDA
sensor_enemy = UltrasonicSensor(Port.S2) #INFRAVERMELHO # Esquerda
